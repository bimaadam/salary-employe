<?php
 $pagedesc = "Data Jurnal";
 include("sess_check.php");

	if(isset($_GET['jur'])) {
		$sql = "SELECT jurnal.*, akun.* FROM jurnal, akun 
				WHERE jurnal.akun_kode=akun.akun_kode 
				AND jurnal.jurnal_trx='". $_GET['jur'] ."'";
		$ress = mysqli_query($conn, $sql);
		$data = mysqli_fetch_array($ress);
	}

	if(isset($_POST['perbarui'])) {
		$trx = addslashes($_POST['trx']);
		$akun = addslashes($_POST['akun']);
		...
		$ress = mysqli_query($conn, $sql);
				...
				//	echo("Error description: " . mysqli_error($conn));
					...
					echo "<script type='text/javascript'> document.location = 'jurnal_edit.php?jur=$trx'; </script>";
				}			
	}
?>
<!DOCTYPE html>
				...
				<div class="row">
					<div class="col-lg-12 col-md-12">										
						...
						<div class="form-group row">
							<label class="col-form-label col-sm-3" align="right">Nomor Transaksi</label>
							<div class="col-sm-6">
								<input type="text" name="trx" class="form-control" value="<?php echo $data['jurnal_trx'];?>" readonly>
							</div>
						</div>
						<div class="form-group row">
							<label class="col-form-label col-sm-3" align="right">Akun</label>
							<div class="col-sm-6">
								<select class="form-control" name="akun" required>
								<?php
									$mySql = "SELECT * FROM akun ORDER BY akun_nama ASC";
						...
						<div class="form-group row">
							<label class="col-form-label col-sm-3" align="right">Nomor Referensi</label>
							<div class="col-sm-6">
								<input type="text" name="ref" class="form-control" value="<?php echo $data['jurnal_reff'];?>" required>
							</div>
						</div>
						<div class="form-group row">
							<label class="col-form-label col-sm-3" align="right">Tanggal</label>
							<div class="col-sm-6">
								<input type="date" name="tgl" class="form-control" value="<?php echo $data['jurnal_tgl'];?>" required>
							</div>
						</div>
						<div class="form-group row">
							<label class="col-form-label col-sm-3" align="right">Jumlah</label>
							<div class="col-sm-6">
								<input type="number" name="jml" min="0" class="form-control" placeholder="Jumlah" value="<?php echo $data['jurnal_jml'];?>" required>
							</div>
						</div>
						<div class="form-group row">
							<label class="col-form-label col-sm-3" align="right">Keterangan</label>
							<div class="col-sm-6">
								<textarea name="ket" class="form-control" required><?php echo $data['jurnal_ket'];?></textarea>
							</div>
						</div>
						<hr/>
						</div>
						<div class="panel-footer">
							...
							<a href="jurnal.php" class="btn btn-info">Batal</a>
						</div>
						</div><!-- /.panel -->
						</form>
					</div><!-- /.col-lg-12 -->
